  ...
  try {
    ...
    if (!organisation || !foodType || !quantity) {
      return res.status(400).json({
        message: "Organisation, food type and quantity are required",
      });
    }
...
export const getNearbyOrganisations = async (req, res) => {
  try {
    ...
    if (!donor.location?.state || !donor.location?.district) {
      return res.status(400).json({
        message: "Donor location is missing",
      });
    }

    const organisations = await Organisation.find({
      isAdminVerified: true,
      "location.state": donor.location.state,
      "location.district": donor.location.district,
    })
      .select("-password")
      .sort({
        createdAt: -1,
